...
需要 **Python 3.11+** 與 [Poetry](https://python-poetry.org/)：
```bash
poetry install
```
...
使用 `scripts/setup_mcp.py` 統一管理 MCP 設定（推薦）：

```bash
# 完整設定：啟動 FalkorDB + 驗證 API Key + 寫入 Claude Desktop 設定
python scripts/setup_mcp.py

# 在 Claude Code 全域啟用（所有專案皆可用）
python scripts/setup_mcp.py --cc-enable

# 停用
python scripts/setup_mcp.py --cc-disable

# 查看目前狀態（FalkorDB / API Key / Claude Desktop / Claude Code）
