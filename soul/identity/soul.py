...
logger = logging.getLogger("soul.identity")
        ...
        if inject_global_rules:
            ...
            if global_claude_md.exists():
                try:
                    ...
                    logger.warning("CLAUDE.md 編碼錯誤（預期 UTF-8）：%s", global_claude_md)
                except PermissionError:
                    logger.warning("無讀取 CLAUDE.md 的權限：%s", global_claude_md)
                except Exception as e:
                    logger.debug("載入 CLAUDE.md 失敗：%s", e)

            # ── 注入 Claude Code Auto Memory（~/.claude/projects/.../memory/MEMORY.md）──
            try:
                project_root = Path(settings.soul_project_root).resolve()
                encoded = str(project_root).replace("/", "-")
                auto_memory_path = Path.home() / ".claude" / "projects" / encoded / "memory" / "MEMORY.md"
                if auto_memory_path.exists():
                    auto_memory = auto_memory_path.read_text(encoding="utf-8").strip()
                    if auto_memory:
                        prompt_parts += [
                            "",
                            "## 專案自動記憶（來自 Claude Code Auto Memory）",
                            auto_memory,
                        ]
            except UnicodeDecodeError:
                logger.warning("MEMORY.md 編碼錯誤：%s", auto_memory_path)
            except PermissionError:
                logger.warning("無讀取 MEMORY.md 的權限：%s", auto_memory_path)
            except Exception as e:
                logger.debug("載入 Auto Memory 失敗：%s", e)
...
class SoulLoader:
    ...
    def _update_frontmatter(self, updates: dict[str, Any]) -> None:
        """就地更新 SOUL.md YAML frontmatter 的特定欄位（atomic write 避免競態）。"""
        if not self._path.exists():
            return
        ...
        # Atomic write：先寫入同目錄的臨時檔，再 rename（os.replace 是原子操作）
        dir_ = self._path.parent
        fd, tmp_path = tempfile.mkstemp(dir=dir_, prefix=".soul_tmp_")
        try:
